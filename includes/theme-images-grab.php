	...
	if ( count( $image_data ) > 0 ) {
		...
		if ( $thumb_id != 0 ) {
			...
			if ( count( $image_sizes ) > 0 ) {
				...
				$html_image_output .= '<img data-src="' . $sharped_images[3] . '" src="' . $sharped_images[1] . '" alt="' . $attachment_alt . '" class="lazy" />';
				$html_image_output .= '</picture>';
	...
	if ( count( $image_data ) > 0 ) {
		...
		if ( $thumb_id != '' ) {
			...
			if ( count( $image_sizes ) > 0 ) {
				...
				$html_image_output .= '<img src="' . $sharped_images[3] . '" alt="' . $attachment_alt . '" />';
				$html_image_output .= '</picture>';
				$html_image_output .= '</div>';
				echo $html_image_output;
			}
	...
	if ( count( $image_data ) > 0 ) {
		...
		if ( $thumb_id != '' ) {
			...
			if ( count( $image_sizes ) > 0 ) {
				...
				$html_image_output .= '<source media="(max-width: 1920px)" srcset="' . $sharped_images[1] . '">';
				$html_image_output .= '<source media="(min-width: 1921px)" srcset="' . $sharped_images[0] . '">';
				$html_image_output .= '<img data-lazy="' . $sharped_images[3] . '" alt="' . $attachment_alt . '" />';
				$html_image_output .= '</picture>';
				$html_image_output .= '</div>';
				echo $html_image_output;
			}
